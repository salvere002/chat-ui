...
const DeepResearchToggle: React.FC = () => {
  ...
  return (
    <button
      className={`flex items-center justify-center px-2 py-1 border rounded-full cursor-pointer transition-all duration-200 text-xs w-[130px] gap-1 min-h-[24px] focus:outline-none focus:shadow-[0_0_0_2px_var(--color-accent-light)] ${
        deepResearchEnabled
          ? 'bg-accent-primary border-accent-primary text-text-inverse hover:bg-accent-hover'
          : 'bg-bg-primary border-border-secondary text-text-primary hover:bg-bg-secondary hover:border-border-hover'
      }`}
