...
export function dynamicProxyPlugin(): Plugin {
  return {
    ...
    configureServer(server) {
      const logger = (server as any)?.config?.logger ?? console;
        ...
        try {
          ...
          const proxyReq = client.request(requestOptions, (proxyRes) => {
            try {
              ...
              logger.error(`Proxy response handling error (preview): ${e}`);
              if (!res.headersSent) {
                res.writeHead(500, { 'Content-Type': 'text/plain' });
              }
              res.end('Proxy response error');
          ...
          proxyReq.on('error', (err) => {
            logger.error(`Proxy error (preview): ${err}`);
            if (!res.headersSent) {
              res.writeHead(500, { 'Content-Type': 'text/plain' });
            }
            res.end('Proxy error: ' + (err as Error).message);
          ...
          logger.error(`Proxy middleware exception (preview): ${err}`);
          res.statusCode = 500;
          res.setHeader('Content-Type', 'text/plain');
          res.end('Internal proxy error');
        }
